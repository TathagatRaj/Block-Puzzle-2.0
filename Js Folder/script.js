...
//create three div containing block images (stone)
$(document).ready(function () {
    function createDiv(id) {
        const div = $("<div>");
        div.attr("id", id);
        div.addClass("stoneBlock")
        return div;
    }

    //creating block images (stone)
    function createImg(src, width, id, container, block_class) {
        const img = $("<img>");
        img.attr("src", src);
        img.attr("width", width);
    ...
    //Calling both the above funtion (stone)
    function generateBlocks() {
        for (let i = 1; i <= 3; i++) {
            $(`#stoneBlock${i}`).append(createDiv(`Stone_shape${i}_container`));
        }

        for (let j = 1; j <= 3; j++) {
            const shapeCollector = `#Stone_shape${j}_container`;
            const stoneArray = `Stone_shape${j}_block`;

            for (let i = 1; i <= 9; i++) {
                createImg("Images/crystal_block.png", "30px", `${stoneArray}_${i}`, shapeCollector, `StoneshowBlocks${j}`);
            }
        }
    }

    // Calling the function when the page loads (stone)
    (function () {
        generateBlocks();
    })();

    //stone Grid block images
    function stone_Grid(src, width, id, container, img_class) {
        const img = $("<IMG>");
        img.attr("src", src);
        img.attr("width", width);
        img.attr("id", id);
    ...
    for (let i = 1; i <= 100; i++) {
        stone_Grid("Images/crystal_block.png", "39.2px", `StoneGridImg${i}`, "#crystal_block_grid2", `stoneGridImgs Row-${Math.ceil(i / 10)} Col-${Math.abs(i) % 10}`);
    };

    // for dragging the block shapes (stone)
    function getBlockCoordinates2(stoneShape) {
        const blocks = [];
        for (let i = 1; i <= 9; i++) {
            const block = document.getElementById(`${stoneShape}_block_${i}`);
            if (block) {
                const rect = block.getBoundingClientRect();
                const BlockX = rect.left + rect.width / 2;
                const BlockY = rect.top + rect.height / 2;
    ...
    const VisibleGridImg2 = [];
    const TotalPlacedBlocks2 = [];
    let TotalLength2;
    function updateGridImages2(gridImgContainer, blocks, targetArea) {
        const TargetArea = targetArea.getBoundingClientRect();
        const TAleft = TargetArea.left;
        const TAright = TargetArea.right;
        const TAtop = TargetArea.top;
        ...
        let GILeft2 = [], GIRight2 = [], GITop2 = [], GIBottom2 = [], Visible_GIBottom2 = [], Visible_GILeft2 = [], Visible_GIRight2 = [], Visible_GITop2 = [];
...
function ThreeBlocks() {
    ...
    for (let j = 0; j < Shapes.length; j++) {
        ...
        for (let i = 0; i < currentShape.length; i++) {
            ...
            // console.log(`#${shapeContainer}_block_${currentShape[i]}`);
        };
    };
    //----------------------------------------------------------
    function CheckSpace() {
        // for (let j = 1; j <= 9; j++) {
        //     if (document.getElementById(`Wood_shape1_block_${j}`).style.visibility === "visible") {
        //         console.log(`Wood_shape1_block_${j} is visible`);
        //         for (let i = 1; i <= 100; i++) {
        //             if (!$(`#WoodGridImg${i}`).hasClass("MoreOpacity")) {
        //                 console.log("atleast one block is invisible");
        //             }
        //         }
        //     } else {
        //         console.log(`Wood_shape1_block_${j} is not visible`);
        //     }
        //     console.log("---------------------------------------------------");
        // }
        ...
        //     const block1 = document.getElementById(`Wood_shape1_block_${pair.shape1}`);
        //     const block2 = document.getElementById(`Wood_shape1_block_${pair.shape2}`);

        //     if (block1.style.visibility === "visible" && block2.style.visibility === "visible") {
        //         for (let start = 1; start <= 91; start += 10) {
        //             for (let k = start; k < start + 9; k++) {
        //                 if (!$(`#WoodGridImg${k}`).hasClass("MoreOpacity") && !$(`#WoodGridImg${k + 1}`).hasClass("MoreOpacity")) {
        //                     console.log(`At least two blocks are invisible for ${pair.shape1} & ${pair.shape2}-------------!!!!!YEAH`);
        //                 }                    
        //             }
        //         }

        //     } else {
        //         console.log(`Both ${pair.shape1} & ${pair.shape2} Block Shape is invisible`);
        //     }
        //     console.log("*****************");
        // });
        ...
        //                 if (!$(`#WoodGridImg${k}`).hasClass("MoreOpacity") && !$(`#WoodGridImg${k + 2}`).hasClass("MoreOpacity")) {
        //                     console.log(`At least two blocks are invisible for ${pair.shape1} & ${pair.shape2}-------------!!!!!YEAH`);
        //                 }                    
        //             }
        //         }
        ...
        // const ThreePair = [
        //     { shape1: 1, shape2: 2, shape3: 3},
        //     { shape1: 4, shape2: 5, shape3: 6},
        //     { shape1: 7, shape2: 8, shape3: 9}
        // ];

        // ThreePair.forEach(pair => {
        //     const block1 = document.getElementById(`Wood_shape1_block_${pair.shape1}`);
        //     const block2 = document.getElementById(`Wood_shape1_block_${pair.shape2}`);
        //     const block3 = document.getElementById(`Wood_shape1_block_${pair.shape3}`);

        //     if (block1.style.visibility === "visible" && block2.style.visibility === "visible" && block3.style.visibility === "visible") {
        //         for (let start = 1; start <= 91; start += 10) {
        //             for (let k = start; k < start + 8; k++) {
        //                 if (!$(`#WoodGridImg${k}`).hasClass("MoreOpacity") && !$(`#WoodGridImg${k + 1}`).hasClass("MoreOpacity") && !$(`#WoodGridImg${k + 2}`).hasClass("MoreOpacity")) {
        //                     console.log(`At least three blocks are invisible for ${pair.shape1} & ${pair.shape2} & ${pair.shape3}-------------!!!!!YEAH`);
        //                 }                    
        //             }
        //         }
        //     } else {
        //         console.log(`All ${pair.shape1} & ${pair.shape2} & ${pair.shape3} Block Shape is invisible`);
        //     }
        //     console.log("*****************");
        // })

        const block1 = document.getElementById("Wood_shape1_block_1");
        const block2 = document.getElementById("Wood_shape1_block_2");
        const block3 = document.getElementById("Wood_shape1_block_3");
        const block4 = document.getElementById("Wood_shape1_block_4");
        const block5 = document.getElementById("Wood_shape1_block_5");
        const block6 = document.getElementById("Wood_shape1_block_6");
        if (block1.style.visibility === "visible" && block2.style.visibility === "visible" && block3.style.visibility === "visible") {
            for (let start = 1; start <= 91; start += 10) {
                for (let k = start; k < start + 8; k++) {
                    if (!$(`#WoodGridImg${k}`).hasClass("MoreOpacity") && !$(`#WoodGridImg${k + 1}`).hasClass("MoreOpacity") && !$(`#WoodGridImg${k + 2}`).hasClass("MoreOpacity")) {
                        console.log(`All three blocks are visible -------------!!!!!YEAH`);
                    }
                }
            }
        }
        else if (block1.style.visibility === "visible" && block2.style.visibility === "visible") {
            for (let start = 1; start <= 91; start += 10) {
                for (let k = start; k < start + 9; k++) {
                    if (!$(`#WoodGridImg${k}`).hasClass("MoreOpacity") && !$(`#WoodGridImg${k + 1}`).hasClass("MoreOpacity")) {
                        console.log(`Block 1 and 2 is visible-------------!!!!!YEAH`);
                    }
                }
            }
        } else if (block2.style.visibility === "visible" && block3.style.visibility === "visible") {
            for (let start = 1; start <= 91; start += 10) {
                for (let k = start; k < start + 9; k++) {
                    if (!$(`#WoodGridImg${k}`).hasClass("MoreOpacity") && !$(`#WoodGridImg${k + 1}`).hasClass("MoreOpacity")) {
                        console.log(`Block 2 and 3 is visible-------------!!!!!YEAH`);
                    }
                }
            }
        } else if (block1.style.visibility === "visible" && block3.style.visibility === "visible") {
            for (let start = 1; start <= 91; start += 10) {
                for (let k = start; k < start + 8; k++) {
                    if (!$(`#WoodGridImg${k}`).hasClass("MoreOpacity") && !$(`#WoodGridImg${k + 2}`).hasClass("MoreOpacity")) {
                        console.log(`Block 1 and 3 is visible-------------!!!!!YEAH`);
                    }
                }
            }
        } else if (block1.style.visibility === "visible") {
            for (let start = 1; start <= 91; start += 10) {
                for (let k = start; k < start + 10; k++) {
                    if (!$(`#WoodGridImg${k}`).hasClass("MoreOpacity")) {
                        console.log(`Block 1 is visible-------------!!!!!YEAH`);
                    }
                }
            }
        } else if (block2.style.visibility === "visible") {
            for (let start = 1; start <= 91; start += 10) {
                for (let k = start; k < start + 10; k++) {
                    if (!$(`#WoodGridImg${k}`).hasClass("MoreOpacity")) {
                        console.log(`Block 2 is visible-------------!!!!!YEAH`);
                    }
                }
            }
        } else if (block3.style.visibility === "visible") {
            for (let start = 1; start <= 91; start += 10) {
                for (let k = start; k < start + 10; k++) {
                    if (!$(`#WoodGridImg${k}`).hasClass("MoreOpacity")) {
                        console.log(`Block 3 is visible-------------!!!!!YEAH`);
                    }
                }
            }
        }
        else {
            console.log(` Block Shape is not visible`);
        }
        console.log("*****************");


    }
    window.GlobalCheckSpace = CheckSpace;
    //----------------------------------------------------------
    // function CheckSpace() {
    //     console.log("Yeah Checking")
    //     let divToMove = document.getElementById("shape1_container");
    //     let TargetStartPos = document.getElementById("WoodGridImg1");
    //     let startPos = TargetStartPos.getBoundingClientRect();
    //     let TargetEndPos = document.getElementById("WoodGridImg10");
    //     let endPos = TargetEndPos.getBoundingClientRect();

    //     let deltaX = endPos.left - startPos.left;
    //     let deltaY = endPos.top - startPos.top;

    //     divToMove.style.position = 'absolute'; 
    //     divToMove.style.left = `${startPos.left}px`;
    //     divToMove.style.top = `${startPos.top}px`;

    //     requestAnimationFrame(() => {
    //         divToMove.style.transition = "transform 5s ease"; // Set duration here
    //         divToMove.style.transform = `translate(${deltaX}px, ${deltaY}px)`;
    //     });
    // }
    // CheckSpace();
